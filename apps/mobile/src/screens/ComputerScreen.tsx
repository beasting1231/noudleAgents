...
export function ComputerScreen({ agents, config, connected }: { agents: Agent[]; config: InstanceConfig; connected: boolean }) {
  ...
  const create = async () => {
    ...
    try { updateSession(await api.createComputer({})); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not start computer"); } finally { setBusy(null); }
  };
  ...
  if (!activeSession && connected) return <StateNotice icon="desktop-outline" title="No computer sessions" detail="Start the shared browser computer, then watch or take control from this phone." action={<Button label="Start computer" icon="add" variant="primary" loading={busy === "create"} onPress={() => void create()} />} />;
